...
struct Initiator: sc_module{
   ...
   const char *data[5]={"Hello", "Shetty", "Pinku", "Boss", "Me"};
        
        uint64_t addresses[5]={0,4,8,12,13};
        for(int i=0;i<5;i++){
        trans.set_command(TLM_WRITE_COMMAND);
        trans.set_address(addresses[i]);
        unsigned int data_length=strlen(data[i])+1;
        trans.set_data_length(data_length);
        trans.set_data_ptr(reinterpret_cast<unsigned char*>(const_cast<char*>(data[i])));
        ...
        for(int i=0;i<5;i++)
        cout<<"Transaction completed: "<<data[i]<<endl;
    }
};
